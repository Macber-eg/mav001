-- Fix memory system SQL queries with explicit table aliases
...
BEGIN
  ...
  WHERE
    ...
    AND m.company_id = (SELECT company_id FROM user_company)
...
-- Update memory search policies to use explicit table aliases
...
-- Update memory vectors policies to use explicit table aliases
...
-- Create helper function to get company ID once
